...
BASE_FILE = FINAL_DATA_DIR / "models_final_provider_verified.json"
PATCH_FILE = FINAL_DATA_DIR / "models_remaining_provider_patched.json"

OUTPUT_FILE = FINAL_DATA_DIR / "models_final_provider_complete.json"
REPORT_FILE = FINAL_DATA_DIR / "merge_remaining_provider_patch_report.txt"
...
def main():
    ...
    for base_record in base_records:
        ...
        if patch_record is not None:
            ...
            for field, value in patch_record.items():
                ...
                # Never overwrite already populated evidence.
    ...
    output = {
        "metadata": {
            **(base_payload.get("metadata") or {}),
            "dataset_name": (
                "AIOrbit Models Final Provider Complete Dataset"
            ),
            "source_files": [
                BASE_FILE.name,
                PATCH_FILE.name,
    ...
    report = [
        "AIOrbit Models - Remaining Provider Patch Merge",
